...
### Overall Score: {fit_score.fit_score}/100

### Score Breakdown
{chr(10).join([f"- **{category.replace('_', ' ').title()}:** {score}/100" for category, score in (fit_score.score_breakdown.items() if hasattr(fit_score, 'score_breakdown') and fit_score.score_breakdown else [])]) if hasattr(fit_score, 'score_breakdown') and fit_score.score_breakdown else "No detailed breakdown available"}

### Detailed Analysis
{fit_score.detailed_feedback if hasattr(fit_score, 'detailed_feedback') else "No detailed feedback available"}
...
### Confidence Level: {fit_score.confidence_level}

---
*Generated by Job Application Automator using GPT-OSS 20B for personalization and Llama 3.2 for fit assessment*
"""
...
def main():
    ...
    try:
        ...
        # Assess fit score - using full content since we're using Llama 3.2
        fit_score = ollama_client.assess_fit_score(
            resume_content=resume_text,  # Full resume content
            job_posting=job_posting,
            cover_letter=cover_letter_result.markdown_content  # Full cover letter content
        )
        ...
        # Display detailed feedback preview
        if hasattr(fit_score, 'detailed_feedback') and fit_score.detailed_feedback:
            print(f"   📝 Detailed Analysis: {fit_score.detailed_feedback[:200]}...")
            print(f"      (Full analysis saved in summary report)")
        ...
        notion_client.create_job_application({
            ...
            "url": job_posting.url,
            "summary": f"""Job application for {job_posting.title} at {job_posting.company} - Generated on {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}.

Location: {job_posting.location}
Skills Required: {', '.join(job_posting.skills)}

Key Personalizations Made:
{chr(10).join([f"• {change}" for change in resume_result.key_changes[:5]])}

Cover Letter Tone: {cover_letter_result.tone}
Skills Emphasized: {', '.join(cover_letter_result.skills_highlighted)}

Generated using Job Application Automator with GPT-OSS 20B and Llama 3.2 for enhanced fit assessment.""",
            "key_strengths": fit_score.strengths[:5],  # Top 5 strengths
            "recommendations": fit_score.recommendations[:5],  # Top 5 recommendations
            "missing_skills": fit_score.missing_skills[:3] if hasattr(fit_score, 'missing_skills') else [],
            "location": job_posting.location,
            "skills_required": job_posting.skills[:10],  # Top 10 skills
            "detailed_feedback": fit_score.detailed_feedback if hasattr(fit_score, 'detailed_feedback') else "",
            "score_breakdown": fit_score.score_breakdown if hasattr(fit_score, 'score_breakdown') else {}
        })
